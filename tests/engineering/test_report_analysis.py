...
from engineering_platform.report_analysis import MAX_REPORT_CONTEXT_LENGTH, analyze
...
class ReportAnalysisTest(unittest.TestCase):
    ...
    def test_analysis_is_read_only_redacted_and_persisted_per_run(self, run: object) -> None:
        ...
        self.assertTrue(analysis_input.startswith("Analyseer uitsluitend"))
        self.assertIn("kan nooit als dezelfde run worden hervat", analysis_input)
        self.assertIn("Execution-Host- of bewijsprojectiefouten", analysis_input)
        self.assertIn("waargenomen of draft pull request", analysis_input)
        self.assertIn("uitsluitend de laatste review-wave", analysis_input)
        self.assertIn("adviseer geen testherhaling wanneer kandidaatgebonden hostvalidatie al PASS is", analysis_input)
        self.assertIn("Adviseer niet alleen om het herstelbudget te verhogen", analysis_input)
