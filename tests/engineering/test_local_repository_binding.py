...
class LocalRepositoryBindingTests(unittest.TestCase):
    ...
    def test_fresh_schema_44_has_no_bindings_and_integrity_passes(self) -> None:
        ...
        self.assertEqual(server.validate_store(self.data_root, identity)["schema_version"], 44)
    ...
    def test_schema_43_migrates_without_inferred_binding(self) -> None:
        ...
        with sqlite3.connect(db) as connection:
            self.assertEqual(connection.execute("SELECT MAX(version) FROM engineering_schema_migrations").fetchone()[0], 44)
            self.assertEqual(connection.execute("SELECT COUNT(*) FROM ep_local_repository_bindings").fetchone()[0], 0)
