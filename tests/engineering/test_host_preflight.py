...
from engineering_platform import host_preflight
...
class HostPreflightTest(unittest.TestCase):
    ...
    def _execute(self):
        ...
        with patch("engineering_platform.platform_api.engineering_platform_codex_cli_prefix", return_value=managed_prefix), patch(
            "engineering_platform.providers.subprocess.run",
    ...
    def test_insufficient_disk_space_fails_closed(self) -> None:
        ...
        with patch("engineering_platform.host_preflight.shutil.disk_usage", return_value=usage), patch.dict(
    ...
    def test_unwritable_reports_directory_fails_closed(self) -> None:
        ...
        with patch("engineering_platform.host_preflight._writable", side_effect=lambda path: False if path.name == "reports" else original(path)):
            result = self._execute()
        self.assertEqual(result.outcome, "FAIL")
    ...
    def test_sqlite_unavailable_fails_without_corrupting_state(self) -> None:
        with patch("engineering_platform.host_preflight.open_storage", side_effect=sqlite3.DatabaseError("unavailable")):
            result = self._execute()
        self.assertEqual(result.outcome, "FAIL")
    ...
    def test_configuration_unavailable_fails_closed(self) -> None:
        with patch("engineering_platform.host_preflight.PlatformConfiguration.load", side_effect=PlatformConfigurationError("missing")):
            result = self._execute()
        self.assertEqual(result.outcome, "FAIL")
