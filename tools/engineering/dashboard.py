...
function estimate(x){const phase=x.current_phase||"";if(phase==="INITIALIZE")return "Voorbereiding: doorgaans minder dan 1 minuut.";if(["EXECUTE_AGENT","REPAIR_AGENT"].includes(phase)){const [minimum,maximum]=executionRange(x);if(!promptStartedAt)return "Indicatieve totale duur: ongeveer "+minimum+"–"+maximum+" minuten, op basis van promptomvang en fase. Live Codex-voortgang is niet beschikbaar.";const elapsed=Math.max(0,Math.floor((Date.now()-promptStartedAt)/60000)),remainingMinimum=Math.max(1,minimum-elapsed),remainingMaximum=Math.max(remainingMinimum,maximum-elapsed);return "Indicatief resterend: ongeveer "+remainingMinimum+"–"+remainingMaximum+" minuten ("+elapsed+" "+pluralMinutes(elapsed)+" verstreken). Gebaseerd op promptomvang, fase en verstreken tijd; geen live Codex-voortgang of tokenverbruik."}if(phase==="FINALIZE_AGENT")return "Finalisatie in uitvoering. De resterende tijd is pas betrouwbaar met live Codex-voortgang.";if(phase==="REPOSITORY_CLEANUP")return "Opschoning in uitvoering. De resterende tijd hangt af van de lokale repository.";if(phase==="WAIT_FOR_TERMINAL_EVIDENCE")return "Wacht op externe verificatie; geen betrouwbare ETA.";if(phase==="COMPLETE")return "Voltooid.";if(["BLOCKED","FAILED"].includes(phase))return "Gestopt; actie nodig.";return "Nog niet beschikbaar."}
...
function promptStarted(){fetch("/api/prompt-started").then(x=>x.json()).then(x=>{promptStartedAt=x.started_at?Date.parse(x.started_at):undefined;$("promptStarted").textContent=promptStartedAt?formatTime.format(new Date(promptStartedAt)):"Niet beschikbaar";if(latestStatus)$("executionEstimate").textContent=estimate(latestStatus)}).catch(()=>{$("promptStarted").textContent="Niet beschikbaar";promptStartedAt=undefined;if(latestStatus)$("executionEstimate").textContent=estimate(latestStatus)})}
