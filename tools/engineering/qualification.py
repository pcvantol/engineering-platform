...
SCENARIOS = tuple(
    QualificationScenario(capability, "Deterministic local contract passes; failures report evidence without changing lifecycle authority.")
    for capability in (
        "Repository Initialization", "Checkpoint Resume", "Implementation Lifecycle", "Validation Loop",
        "Repair Loop", "Owner Authorization", "Ready For Review", "Automatic Merge",
        "Repository Reconciliation", "Finalization", "Repository Cleanup", "Engineering Memory",
        "Progress Reporting", "Engineering Reports", "Capability-aware Reviewers", "Diagnostics",
        "BLOCKED Recovery", "Failure Recovery", "Long-running Transactions",
    )
)
...
def execute_qualification(root: Path, checks: dict[str, bool] | None = None) -> dict[str, object]:
    ...
    manifest = EngineeringPlatformManifest.load(root / "tools" / "engineering" / "ENGINEERING_PLATFORM_VERSION.json")
    supplied = checks or {}
    results = []
    for scenario in SCENARIOS:
        passed = supplied.get(scenario.capability, _default_check(root, scenario.capability))
        results.append({"capability": scenario.capability, "status": "PASS" if passed else "FAIL", "duration_ms": 0, "diagnostic": None if passed else "Scenario contract failed.", "evidence": scenario.expected_behavior})
...
def dashboard(report: dict[str, object]) -> str:
    ...
    return "\n".join((
        ...
        f"Failures: {report['failures']}", f"Blocked: {report['blocked']}",
...
def _default_check(root: Path, capability: str) -> bool:
    contracts = {
        ...
        "Capability-aware Reviewers": (root / "tools" / "engineering" / "capability_review.py").is_file(),
    }
...
def _write_report(root: Path, report: dict[str, object]) -> None:
    ...
    (directory / f"qualification-{stamp}.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
