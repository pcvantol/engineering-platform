...
def stable_prompt(path: Path, interval: float = 1.0) -> str | None:
    ...
    if path.name.startswith(".") or path.suffix.lower() not in {".txt", ".md"} or path.is_symlink() or not path.is_file():
        return None
    try:
        before = path.stat()
    except OSError:
...
def discover(root: Path, interval: float = 0.0) -> list[Path]:
    ...
    return [path for path in sorted(inbox.iterdir(), key=lambda item: (item.stat().st_mtime_ns, item.name)) if stable_prompt(path, interval) is not None]
...
def status(root: Path, state: str, **details: object) -> None:
    ...
    payload = {"watcher_state": state, "watcher_version": WATCHER_VERSION, "last_update": datetime.now(timezone.utc).isoformat(), **{key: _safe_detail(value) for key, value in details.items()}}
    temporary = root / ".status.tmp"
    temporary.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    os.replace(temporary, root / "status.json")
    (root / "status.md").write_text(
        "# DJConnect Engineering Inbox\n\n"
        f"State: `{state}`\n\nActive job: `{details.get('job_id', 'none')}`\n\n"
        f"Queue: `{details.get('queued_jobs', 0)}`\n\nDiagnostic: {_safe_detail(details.get('diagnostic', 'none'))}\n",
        encoding="utf-8",
    )
...
def _already_seen(areas: dict[str, Path], job_id: str) -> bool:
    return any(next(areas[name].glob(f"{job_id}__*"), None) is not None for name in ("Running", "Completed", "Failed"))
...
def once(repo: Path, root: Path, interval: float = 1.0) -> int:
    ...
    with _lock(repo):
        ...
        if _active_transaction(repo):
            status(root, "WAITING_FOR_REPOSITORY", queued_jobs=len(candidates), diagnostic="An existing engineering transaction remains active.")
            return 0
        source, content = candidates[0]
        job_id, run_id, digest = _job_id(source, content)
        if _already_seen(areas, job_id):
            status(root, "WATCHER_IDLE", queued_jobs=len(candidates) - 1, job_id=job_id, diagnostic="Duplicate job digest remains recorded.")
            return 0
        ...
        (local / "job.json").write_text(json.dumps({"job_id": job_id, "run_id": run_id, "filename": source.name, "digest": digest, "received_at": datetime.now(timezone.utc).isoformat()}, indent=2) + "\n", encoding="utf-8")
        phase, _ = _runner_result(repo, run_id)
        arguments = [str(repo / "tools/engineering/dj-engineer"), str(prompt.relative_to(repo)), "--owner-authorized", "--run-id", run_id]
        if phase and phase not in TERMINAL_PHASES:
            arguments.append("--resume")
        status(root, "RUNNER_STARTING", job_id=job_id, run_id=run_id, queued_jobs=len(candidates) - 1)
        ...
        delivered = None
        if report:
            status(root, "REPORT_PUBLISHING", job_id=job_id, run_id=run_id, queued_jobs=len(candidates) - 1)
            delivered = areas["Reports"] / f"{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}_{job_id}_{run_id}.md"
        ...
        final_state = "JOB_COMPLETED" if successful else ("JOB_BLOCKED" if phase == "BLOCKED" else "JOB_FAILED")
        reason = diagnostic or ("Engineering report was not available for delivery." if completed.returncode == 0 else "Runner ended without a safe terminal report.")
        status(root, final_state, job_id=job_id, run_id=run_id, queued_jobs=len(candidates) - 1, runner_phase=phase, report=str(delivered) if delivered else None, diagnostic=reason, resume_instruction=f"Run dj-engineer with --run-id {run_id} --resume.")
...
def launch_agent(repo: Path) -> Path:
    ...
    destination.write_text(f'<?xml version="1.0" encoding="UTF-8"?><plist version="1.0"><dict><key>Label</key><string>{LABEL}</string><key>ProgramArguments</key><array>{arguments}</array><key>WorkingDirectory</key><string>{repo}</string><key>RunAtLoad</key><true/><key>KeepAlive</key><true/><key>ThrottleInterval</key><integer>15</integer><key>StandardOutPath</key><string>{logs / "inbox.out.log"}</string><key>StandardErrorPath</key><string>{logs / "inbox.err.log"}</string></dict></plist>', encoding="utf-8")
    return destination
...
def doctor(repo: Path, root: Path) -> int:
    ...
    checks = {"repository_runner": (repo / "tools/engineering/dj-engineer").is_file(), "inbox_writable": os.access(areas["Inbox"], os.W_OK), "reports_writable": os.access(areas["Reports"], os.W_OK), "launch_agent": agent.is_file(), "gitignored": ".djconnect/" in (repo / ".gitignore").read_text(encoding="utf-8")}
    state = "REMOTE_ENGINEERING_READY" if all(checks.values()) else "REMOTE_ENGINEERING_DEGRADED"
    print(json.dumps({"state": state, "watcher_version": WATCHER_VERSION, "inbox": str(areas["Inbox"]), "reports": str(areas["Reports"]), "checks": checks}, indent=2))
    return 0 if state == "REMOTE_ENGINEERING_READY" else 1


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("command", choices=("once", "run", "status", "install", "uninstall", "doctor"))
    parser.add_argument("--repo", type=Path, default=Path.cwd())
    parser.add_argument("--icloud-root")
    parser.add_argument("--interval", type=float, default=15)
    args = parser.parse_args(argv)
    repo, root = args.repo.resolve(), cloud_root(args.icloud_root)
    if args.command == "once": return once(repo, root, 0.0)
    if args.command == "run":
        while True:
            try: once(repo, root, 1.0)
            except RuntimeError: status(root, "WAITING_FOR_REPOSITORY", diagnostic="Another watcher owns the local inbox lock.")
            time.sleep(max(5, args.interval))
    if args.command == "status": print((root / "status.md").read_text(encoding="utf-8") if (root / "status.md").exists() else "WATCHER_IDLE"); return 0
    agent = Path.home() / "Library/LaunchAgents" / f"{LABEL}.plist"
    if args.command == "install":
        agent = launch_agent(repo); subprocess.run(("launchctl", "bootout", f"gui/{os.getuid()}", str(agent)), check=False, capture_output=True); subprocess.run(("launchctl", "bootstrap", f"gui/{os.getuid()}", str(agent)), check=False); return 0
    if args.command == "uninstall": subprocess.run(("launchctl", "bootout", f"gui/{os.getuid()}", str(agent)), check=False); agent.unlink(missing_ok=True); return 0
