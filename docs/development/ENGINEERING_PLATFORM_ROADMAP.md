...
3. Qualify OI-5a/OI-5b and the OI-5c admission-consumption boundary before
   adding any mutating operation. Only then add an EP-owned privileged
   account-discovery and service-migration provisioner operation with durable
   runtime slots. That later operation must retain one selected runtime,
   backup/migration/health/cleanup evidence and an explicit machine-scope
   coverage receipt; it must not treat this source contract or a per-user
   LaunchAgent as a completed cutover.
