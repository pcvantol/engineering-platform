...
| OI-3 | engineering-platform | Product-owned consumer readback and exact-candidate assessment | SOURCE_FIXED: #162 `25ffe34`, qualified by #163 `495e196`. The OI-5 resolver convergence retains this OI-3 top-level wire contract while changing only the EP-owned resolver evidence from a legacy LaunchAgent to the system-domain service observer. `operational-update-assess` requires `ACTIVE` readback and exact wheel/version/digest/source-revision preconditions. It does not execute, publish, cut over, migrate or clean an operational installation. |
...
| OI-5a | engineering-platform | System-domain Server service and machine-scope inventory evidence foundation | SOURCE_FIXED: #170 `690f9be`. The bounded source increment defines one future non-root-account LaunchDaemon descriptor with a normalized data root and structural venv launcher, plus a read-only `system-service-inventory` surface. It uses descriptor-bound snapshots for system, shared-user and explicitly declared user service references; malformed, inaccessible and raced observations remain explicit. Account discovery remains incomplete, so it never claims `SINGLE_OPERATIONAL_INSTALLATION_VERIFIED`. It exports no privileged daemon mutation. |
| OI-5b | engineering-platform | Canonical system-service resolver/readback convergence | SOURCE_FIXED: #171 `2255c67`. The official EP diagnostic, qualification, readback, update-assessment, inventory and development-profile boundaries use only the OI-5a system-domain resolver. They retain OI-3's 1.0 top-level response shape, preserve incomplete system-scope evidence, and fail closed to `UNKNOWN` on absent, malformed or observer-mismatched descriptors. The observer must retain the complete canonical system entry (label, plist, launcher, data root and service account) before any official surface may select it; legacy LaunchAgent, `PATH` and `sys.executable` cannot select an operational runtime. This remains read-only and is not a provisioner, service cutover, migration, backup, rollback or cleanup completion. |
| OI-5c | engineering-platform | Admission-bound EP update composition | SOURCE_FIXED: #172 `2993620`. The EP-specific composition no longer accepts a caller-selected target interpreter: it consumes only a typed OI-4c v2 admission and reopens the exact staged candidate from the durable operation binding before quiesce, backup, migration, activation and verification. The v2 admission binds the complete closed old record. On reboot it accepts a `MIGRATED` target record only when it is the deterministic full replacement (preserving channel/roles/desired state, with only target identity plus `ACTIVATING`/pending lifecycle fields changed), rechecks the exact candidate and service selection, then journals `ACTIVATED` through an exact idempotent CAS. A v1 admission may be upgraded only while `PREPARED` and never reaches mutating execution. This remains a source boundary only; it neither enables a live updater nor turns the operation-root candidate into a retained runtime slot, system service, rollback or cleanup completion. |
| OI-5d | engineering-platform | Pure system-installation topology and digest-pinned final runtime-slot transition plan | CANDIDATE_SOURCE_FIX: `codex/ep-system-installation-topology-v1`. A caller supplies one explicit system product root and non-root service account; EP derives disjoint data, runtime, operation, recovery and one machine-lock locations. Exact version/digest/source-revision evidence maps to one final `runtimes/slots/sha256-<digest>/venv/bin/python` target, never an operation candidate or a version-only path. The closed plan classifies only build/download/pip-cache as ephemeral and protects data, runtime and recovery paths. It performs no filesystem, lock, account, artifact, service, migration, backup, rollback, record or cleanup mutation and makes no installed/uniqueness claim. |
...
3. OI-5d first fixes the pure topology, final-slot and recovery/cleanup
   boundary without a mutator. Only then add an EP-owned privileged
   account-discovery and service-migration provisioner operation that acquires
   OI-5d's machine lock, creates the target venv in its final slot, and retains
   one selected runtime plus backup/migration/health/cleanup evidence and an
   explicit machine-scope coverage receipt. It must not treat this source
   contract or a per-user LaunchAgent as a completed cutover.
