...
| PLATFORM | `GET /api/components/{http_ingress,cli_ingress,file_inbox_ingress}/details`, `GET /api/logs/{dashboard,inbox}`, `/api/{process-metrics,usage}` | Platform Components, status popout, logging and File Inbox projection |
