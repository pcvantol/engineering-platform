...
| AC-05 | high | Component consumers duplicated identity inventories. `PLATFORM_COMPONENT_MODEL_COUNT=1` | Route pattern, startup events and transport rendering derive from `platform_components`. Remaining aliases/legacy inventory are tracked by AC-06. | IN_PROGRESS |
| AC-06 | critical | Legacy watcher/dashboard runtime remains packaged and has supported-looking code paths. `SUPPORTED_LEGACY_INBOX_WATCHER_RUNTIME=0` | Consequence-led capability replacement and structural deletion. | IN_PROGRESS |
