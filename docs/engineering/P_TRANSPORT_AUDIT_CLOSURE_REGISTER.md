...
| AC-10 | high | Localization guard omits some Server-generated surface and stale watcher copy. | The Server-owned no-project title, explanation and selector use five-locale keys. CENTRAL-database mutation feedback now also uses catalog keys through the read-only Console translation bridge; a regression guard rejects a return to hard-coded feedback. | IN_PROGRESS — audit remaining Server-rendered values and stale Watcher/Finder actions. |
