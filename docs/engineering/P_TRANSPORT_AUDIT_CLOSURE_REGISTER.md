...
| AC-06 | critical | The legacy Dashboard direct wrapper remains packaged and has supported-looking root/worktree paths. `SUPPORTED_LEGACY_INBOX_WATCHER_RUNTIME=0` | `inbox_watcher.py` and its runtime tests are physically retired; the canonical Server import boundary and authority guard prove it cannot return. Structural separation/removal of the remaining direct Dashboard handler and its root/worktree contracts is still required. | IN_PROGRESS |
