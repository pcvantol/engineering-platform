...
| Human receipt / provenance | **REQUALIFY** | AC-09 schema-52 now provides an immutable, installation-bound receipt→run relation; direct SQLite canaries cover substitution, scope mismatch, rewrite rejection and schema-51 backfill/fail-closed import. | Rebuild the candidate, run the full suite/coverage, then run installed Human Intent and replay canaries on the exact head. |
