...
| AC-10 | high | Localization guard omitted some Server-generated surface and stale watcher copy. | The Server-owned no-project title, explanation and selector use five-locale keys. CENTRAL-database mutation feedback uses catalog keys through the read-only Console translation bridge. Workspace unavailability is neutral Server data and GPU state is a machine code; the browser alone localizes both. Finder, local Inbox-location and runtime-directory actions are absent from supported browser code and rejected by retained routes with `410`. | QUALIFIED — Server Console, transport-authority and five-locale guards PASS. |
