...
| NFR-INSTALL-001 | The target EP operational invariant is `PER_PRODUCT_MAX_OPERATIONAL_RELEASE_INSTALLATIONS = 1` at `MACOS_MACHINE` scope. One selected system-domain EP service, exact artifact/release, structural venv launcher, data root and live health identity must bind to one installation record. The legacy per-user installer/LaunchAgent is interim compatibility only and cannot satisfy this requirement. Existing data is reused only after explicit compatibility evidence, backed up before replacement, and removed only after a second destructive confirmation. | Clean-machine, cross-account/service inventory, concurrent installer, upgrade/migration/rollback, backup/restore, cleanup, fresh-shell and reboot qualification of the selected installed artifact. |
