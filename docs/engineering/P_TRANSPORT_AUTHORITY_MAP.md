...
| historical `inbox_watcher.py` `once`, `run`, `install` commands | HISTORICAL_ONLY | source retired; Git history and the read-only archive migration tool preserve the evidence | no installed module, console script, route or service can invoke a watcher |
