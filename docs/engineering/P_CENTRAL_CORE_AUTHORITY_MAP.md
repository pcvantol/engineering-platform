...
| `execution_host.py`: CENTRAL-bound runner | checkpoints, leases, recovery, provider usage, phase spans, terminal report index | CENTRAL_CANONICAL | CENTRAL_CANONICAL | explicit database/artifact bindings injected from `StateStore` | focused host/recovery/timing tests |
...
| `execution_lease.py`, `provider_recovery.py`: CENTRAL runner route | leases, heartbeats, recovery and retry lineage | CENTRAL_CANONICAL | CENTRAL_CANONICAL | explicit database binding through runner | focused lease/recovery tests |
