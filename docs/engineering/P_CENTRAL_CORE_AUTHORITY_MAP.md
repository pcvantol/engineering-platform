...
| `execution_host.main` | legacy `python -m engineering_platform` entrypoint | direct-host lifecycle | ACTIVE_STANDALONE_OPERATIONAL | explicit `--central-database` is CENTRAL_CANONICAL; omitted option is still REPOSITORY_LOCAL_DB + LOCAL_STATESTORE | remove the omitted-option fallback or isolate the entrypoint behind historical/forensic use |
