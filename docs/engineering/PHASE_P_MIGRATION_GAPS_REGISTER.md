...
| MG-13 | **Legacy names and service topology.** Product code and service definitions retained predecessor variables and LaunchAgent labels. | **CLOSED — P-NEUTRAL** | Active environment variables, labels, defaults and lifecycle targets now use Engineering Platform names.  Explicit migration and provenance references remain read-only and are guarded by the P-NEUTRAL namespace inventory. |
