...
| LR-09 | `_admit_dependabot_pull_requests()` discovers external PRs, writes an envelope into the retired Inbox and records root-bound local admission evidence. It is called only by `inbox_watcher.py`. The replacement now exists: Server-owned `dependabot_producer.py` resolves an explicit CENTRAL `ExternalProducerBinding` and calls canonical `submission_service.submit`. | IN_PROGRESS — CANONICAL_SUCCESSOR_IMPLEMENTED | Complete installed two-project, source-validation, replay/restart and negative-binding qualification; then delete the watcher helper, local `dependabot_admission_events` authority, tests, package artifacts and stale docs before marking resolved. |
