...
| LR-12 | `dashboard.py` exposes a second two-item `dashboard`/`dashboard_relay` inventory for its direct historical HTTP/LaunchAgent runtime, while the Server uses `PLATFORM_COMPONENTS`. | UNRESOLVED | The canonical Relay definition now owns its LaunchAgent label and restart capability. The Server projects Relay health from that lifecycle owner and exposes the bounded PLATFORM restart route; it records the mutation in CENTRAL logging. The remaining direct Dashboard inventory and direct restart route must still be extracted or derived before claiming `DUPLICATED_COMPONENT_INVENTORIES=0` or `USER_MUTATION_WITHOUT_REAL_SERVER_BOUNDARY=0`. |
