...
| LR-12 | `dashboard.py` exposes a second two-item `dashboard`/`dashboard_relay` inventory for its direct historical HTTP/LaunchAgent runtime, while the Server uses `PLATFORM_COMPONENTS`. | UNRESOLVED | The canonical Relay definition now owns the LaunchAgent label, and the Server health projection queries that real lifecycle owner rather than inferring Relay health from Server liveness. The remaining direct Dashboard inventory must still be extracted or derived before claiming `DUPLICATED_COMPONENT_INVENTORIES=0`. |
