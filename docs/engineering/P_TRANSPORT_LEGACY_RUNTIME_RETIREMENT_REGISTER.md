...
| --- | --- | --- | --- |
| LR-01 | `inbox_watcher.py` retains local queue, preflight, retry and launch-agent implementation. It is directly imported by `parity_lifecycle_dispatcher.py`, `emergency_recovery.py`, and lazy direct-dashboard compatibility. | UNRESOLVED | Extract/replace the active dispatcher and recovery dependencies; then remove watcher runtime, its tests, service installation and package artifact. |
