...
| LR-03 | `component_logging.py` and `storage.py` retain `inbox`/`dashboard` local-log compatibility identifiers. The supported writer path now has no rotating-file fallback: unavailable CENTRAL logging emits only a bounded stderr diagnostic and cannot create `.engineering/logs`. | UNRESOLVED | Remove the remaining historical reader/migration identifiers with the direct Dashboard/watcher handler; retain only canonical component identities. |
...
| LR-09 | `_admit_dependabot_pull_requests()` discovers external PRs, writes an envelope into the retired Inbox and records root-bound local admission evidence. It is called only by `inbox_watcher.py`; no canonical Server/CENTRAL equivalent or supported product contract was found. | UNRESOLVED — LEGACY_CAPABILITY_REPLACEMENT_REQUIRED | Do not retain it as watcher compatibility or delete it silently. Define a canonical authenticated external-producer ingress with explicit repository authority and CENTRAL provenance, then qualify it before retiring the helper and its table/tests. |
| LR-11 | `workspace_inbox_api.py` and `human_text_ingress.py` still write a root-selected iCloud Inbox, create repository-local submission evidence and name the watcher as lifecycle owner. Their only production caller is `inbox_watcher.py`; their tests and `ENGINEERING_INBOX_PROTOCOL.md` preserve the same retired model. | UNRESOLVED | The supported successor is the Server-owned `file_inbox.py` plus transport-neutral `submission_intake.py`, documented by `HUMAN_INTENT_FILE_INBOX.md` and qualified through the structured 3×2 and Human Intent 2× gates. Before structural deletion, replace the historical capability-matrix/protocol claims with that successor and prove no supported caller still imports these modules. |
| LR-12 | `dashboard.py` exposes a second two-item `dashboard`/`dashboard_relay` inventory for its direct historical HTTP/LaunchAgent runtime, while the Server uses `PLATFORM_COMPONENTS`. | UNRESOLVED | Retain Dashboard Relay only as a Server-derived access adapter. Extract its direct handler/process inventory or make it derive from the canonical model before claiming `DUPLICATED_COMPONENT_INVENTORIES=0`. |
