...
| --- | --- | --- | --- |
| LR-01 | `inbox_watcher.py` retains local queue, preflight, retry and launch-agent implementation. The canonical lifecycle dispatcher owns run identity and admission evidence and calls the independent host/workspace/capability checks directly; emergency recovery no longer publishes watcher status. The direct Dashboard handler now has no watcher import, label, launch/restart path, local retry route or branch/worktree relocation route. A loopback regression proves those retired routes return 404. | UNRESOLVED | Retire the remaining watcher module, its tests, service-installation/migration consequences and package artifact after the Server presentation split is complete. |
