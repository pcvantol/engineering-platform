...
| --- | --- | --- | --- |
| LR-01 | `inbox_watcher.py` retains 2,642 lines of local queue, preflight, retry, launch-agent and root-state implementation. The canonical lifecycle dispatcher owns run identity and admission evidence and calls the independent host/workspace/capability checks directly; emergency recovery no longer publishes watcher status. The direct Dashboard handler has no watcher import, label, launch/restart path, local retry route or branch/worktree relocation route. A loopback regression proves those retired routes return 404. Reverse-dependency analysis finds no production import: remaining consumers are its historical test suite, iCloud-archive migration tooling and a Dependabot-to-old-Inbox helper. | UNRESOLVED | Split the iCloud migration and determine the canonical Dependabot ingress successor; then remove the watcher module, its historical tests and package artifact. |
