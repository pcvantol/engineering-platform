...
| --- | --- | --- | --- |
| LR-01 | `inbox_watcher.py` retains local queue, preflight, retry and launch-agent implementation. The canonical lifecycle dispatcher no longer imports it: it owns run identity and admission evidence and calls the independent host/workspace/capability checks directly. `emergency_recovery.py` and lazy direct-dashboard compatibility still retain watcher dependencies. | UNRESOLVED | Migrate the remaining recovery/direct-dashboard dependencies; then remove watcher runtime, its tests, service installation and package artifact. |
