...
def generate_terminal_report(
    ...
    provider_usage_lines = (
        ...
        *invocation_lines,
        ...
        f"- Unique Structured PR Results: `{observed(provider_usage.get('historical_unique_pr_results'))}`; coverage `{provider_usage.get('historical_unique_pr_coverage', 'UNAVAILABLE')}`",
        f"- Unique Structured PR Results Lower Bound: `{observed(provider_usage.get('historical_unique_pr_results_lower_bound'))}` (shown only when exact scope-wide uniqueness is unavailable)",
